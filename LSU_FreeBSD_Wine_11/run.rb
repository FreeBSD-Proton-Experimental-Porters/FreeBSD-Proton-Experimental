...
wine64_bin = '/usr/local/wine-proton-11/bin/wine64'
wine32_bin = File.join(I386_PKG_ROOT, 'usr/local/wine-proton-11/bin/wine')

if !File.exist?(wine64_bin)
  perr "#{wine64_bin} doesn't exist!"
  perr "Install wine-proton-11 first."
  exit(1)
end
...
raise if !File.exist?('/usr/local/wine-proton-11/lib/wine/x86_64-windows')
raise if !File.exist?(File.join(I386_PKG_ROOT, 'usr/local/wine-proton-11/lib/wine/i386-windows'))
  ...
  def set_setup_state(state)
    if state != $setup_state
      case state
        ...
        when :manifest
          pwarn "Registering wine-proton-11 as a compatibility tool..."
        when :done
    ...
    Dir.chdir('FreeBSD_Proton') do
      ...
      Dir.chdir("proton_#{PROTON_VERSION}") do
        ...
        set_up_file('proton') do |target|
          ...
          # Remove wine-preloader use in script as it currently segfaults on FreeBSD
          raise if !str.gsub!('#run with winepreloader directly to avoid restart through start.exe', '')
        ...
        set_up_file('files') do |target_dir|
          ...
          set_up_file(File.join("#{target_dir}.tmp", 'bin')) do |target|
            ...
            FileUtils.mkdir_p(target)
            Dir.chdir(target) do
              for file in Dir['/usr/local/wine-proton-11/bin/{msidb,wine,wine64,wineserver}']
                safe_system('ln', '-s', file)
              end
            end
          end
          ...
          if PROTON_VERSION.to_i >= 10
            set_up_file(File.join("#{target_dir}.tmp", 'bin-wow64')) do |target|
              ...
              FileUtils.mkdir_p(target)
              Dir.chdir(target) do
                for file in Dir['/usr/local/wine-proton-11/bin-wow64/{msidb,wine,wineserver}']
                  safe_system('ln', '-s', file)
                end
              end
            end
